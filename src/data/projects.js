export const projectsData = [
  //
  // {
  //     "title": "",
  //     "img": "",
  //     "description": "",
  //     "technologies": ["ReactJs", "MaterialUi", "Css"],
  //     "url": "",
  //     "github": "",
  // },
  {
    title: "Hotel Software",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1704935193/portfolio-Goncal/hotel_goncal_1_qkrqmt.png",
    description:
      "Full-stack project. You can make reservations, with automatic email notifications, and admin functions.",
    technologies: ["React", "MaterialUi", "MongoDB", "NodeJs", "ExpressJs"],
    url: "https://hotel-goncal.netlify.app/",
    github: "https://github.com/goncalhurtado/hotel-software",
    githubBack: "https://github.com/goncalhurtado/hotel-software-backend",
    fullstack: true,
  },
  {
    title: "Home Banking APP",
    img: "https://res.cloudinary.com/dr2iqnauy/image/upload/v1710177605/repos/native/Frame_1_2_bxfnth.png",
    description: "Full-stack project. Homebanking Mobile App",
    technologies: ["React Native", "Expo", "MongoDB", "NodeJs", "ExpressJs"],
    url: "https://github.com/goncalhurtado/banking-app-native",
    github: "https://github.com/goncalhurtado/banking-app-native",
    githubBack: "https://github.com/goncalhurtado/bankapp-backend",
    fullstack: true,
  },
  {
    title: "Qr Generator",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1703195296/portfolio-Goncal/generador_de_Qr_wcm2hm.png",
    description:
      "QR code generator for WhatsApp links, email, and regular URLs, allowing customization of the QR.",
    technologies: ["React", "MaterialUi", "Css", "Javascript"],
    url: "https://generador-de-qr.netlify.app/",
    github: "https://github.com/goncalhurtado/qr-generator",
    fullstack: false,
  },
  {
    title: "Product Designer's Portfolio",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1703195769/portfolio-Goncal/porfolio_tili_cpmelt.png",
    description: "A portfolio for a senior product designer Lu Diaz Spolita.",
    technologies: ["React", "Scss", "Bootstrap"],
    url: "https://tilininia.netlify.app/",
    github: "https://github.com/goncalhurtado/lucila-portfolio",
    fullstack: false,
  },
  {
    title: "Music streaming page",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1695953496/portfolio-Goncal/pinkfi_kvtgxz.png",
    description:
      "Similar to Spotify but with Vanilla JavaScript. With user registration, admin, and music CRUD",
    technologies: ["Html", "Css", "Javascript"],
    url: "https://pinkify-music.netlify.app/",
    github: "https://github.com/goncalhurtado/pinkify-finalmodulo2",
    fullstack: false,
  },
  ,
  {
    title: "Age Wizard",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1705702553/portfolio-Goncal/age_calculator_1_ny0eag.png",
    description:
      "This simple and charming tool allows users to determine their age with precision.",
    technologies: ["React", "Typescript", "Bootstrap", "Scss"],
    url: "https://age-wizard.netlify.app/",
    github: "https://github.com/goncalhurtado/age-calculator",
    fullstack: false,
  },

  {
    title: "Color Notepad",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1703227105/portfolio-Goncal/color_notepad_mvpsnf.png",
    description:
      "You can create, store and choose the color of each note, and the notes are stored in the browser's local storage.",
    technologies: ["React", "Javascript", "Bootstrap", "Scss"],
    url: "https://color-notepad.netlify.app/",
    github: "https://github.com/goncalhurtado/notepad-vox",
    fullstack: false,
  },
  {
    title: "Datamath Calculator",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1703266104/portfolio-Goncal/data_calculator_mcidys.png",
    description:
      "Functional calculator based on the iconic DataMatch Electronic Calculator 2500",
    technologies: ["React", "Javascript", "Css"],
    url: "https://datamath2.netlify.app/",
    github: "https://github.com/goncalhurtado/calculator",
    fullstack: false,
  },
  {
    title: "Slot Machine",
    img: "https://res.cloudinary.com/dhvgi2cmq/image/upload/v1703612473/portfolio-Goncal/slotmachine_1_lcqcbp.png",
    description: "Emulates the gameplay of a classic casino slot machine.",
    technologies: ["React", "Javascript", "Scss"],
    url: "https://react-slot-machine.netlify.app/",
    github: "https://github.com/goncalhurtado/slot-machine-game",
    fullstack: false,
  },
];
